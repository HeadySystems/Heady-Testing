...
const ORS_THRESHOLDS = {
  FULL: 85,       // Full parallelism, aggressive building
  NORMAL: 70,     // Normal operation
  MAINTENANCE: 50, // Reduced load, no new large builds
...
class HCReadiness {
  ...
  loadProbes(configPath) {
    ...
    try {
      if (fs.existsSync(yamlPath)) {
        ...
        console.log(`[hc-readiness] Loaded ${this.probes.length} probes`);
      }
    } catch (err) {
      console.error('[hc-readiness] Failed to load probes:', err.message);
    }
    return this;
  }
  ...
  async assess() {
    ...
    const mode = ors >= ORS_THRESHOLDS.FULL ? 'full'
      : ors >= ORS_THRESHOLDS.NORMAL ? 'normal'
      : ors >= ORS_THRESHOLDS.MAINTENANCE ? 'maintenance'
      : 'recovery';
    ...
    console.log(`[hc-readiness] ORS=${ors} mode=${mode} (${evaluation.passed}/${this.probes.length} probes passed)`);
    return evaluation;
  }
  ...
  async prepare(targetMode = 'normal') {
    ...
    if (evaluation.mode === 'recovery' && targetMode !== 'recovery') {
      actions.push({ action: 'repair', message: 'System in recovery mode — fix failing probes before proceeding' });
      for (const r of evaluation.results.filter(r => !r.passed)) {
        actions.push({ action: 'fix-probe', probe: r.name, error: r.error });
      }
    }

    if (evaluation.mode === 'maintenance' && (targetMode === 'full' || targetMode === 'normal')) {
      actions.push({ action: 'stabilize', message: 'System in maintenance mode — stabilize before scaling' });
    }
  ...
  async _runProbe(probe) {
    ...
    const result = { name: probe.name, type: probe.type, passed: false, latencyMs: 0, error: null };

...
const PII_PATTERNS = [
  ...
  { type: 'API_KEY', pattern: /\b(?:sk|pk|api)[-_][A-Za-z0-9]{20,}\b/gi },
  { type: 'JWT', pattern: /\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\b/g }
];

// ── Injection Patterns ────────────────────────────────────────────────────────
const INJECTION_PATTERNS = [
  { type: 'PROMPT_INJECTION', pattern: /(?:ignore|forget|disregard)\s+(?:all\s+)?(?:previous|above|prior)\s*(?:instructions|rules|prompts)/i },
  { type: 'SYSTEM_OVERRIDE', pattern: /you\s+are\s+now\s+(?:a|an|the)\s+/i },
  { type: 'JAILBREAK', pattern: /(?:DAN|Do\s+Anything\s+Now|\\n\\nHuman:|ADMIN\s+MODE|DEVELOPER\s+MODE)/i },
  { type: 'SQL_INJECTION', pattern: /(?:UNION\s+SELECT|DROP\s+TABLE|;\s*DELETE|OR\s+1\s*=\s*1|--\s*$)/i },
  { type: 'XSS', pattern: /<script|javascript:|on(?:load|error|click)\s*=|eval\s*\(/i }
];
...
export function detectPII(text) {
  ...
  for (const { type, pattern } of PII_PATTERNS) {
    ...
    while ((match = regex.exec(text)) !== null) {
      entities.push({ type, match: match[0], index: match.index });
    }
  }
...
/**
 * Detect injection attempts — prompt injection, SQL injection, XSS, jailbreaks.
 * @param {string} text
 * @returns {{ safe: boolean, threats: Array<{type: string, match: string}> }}
 */
export function detectInjection(text) {
  const threats = [];
  for (const { type, pattern } of INJECTION_PATTERNS) {
    const match = text.match(pattern);
    if (match) threats.push({ type, match: match[0] });
  }
...
export function qualityGate(output, opts = {}) {
  const { minLength = 10, maxLength = 50000, cslThreshold = CSL.INCLUDE } = opts;

  if (!output || typeof output !== 'string') return { passed: false, score: 0, reason: 'Empty output' };
  if (output.length < minLength) return { passed: false, score: 0.1, reason: `Too short: ${output.length} < ${minLength}` };
  if (output.length > maxLength) return { passed: false, score: 0.2, reason: `Too long: ${output.length} > ${maxLength}` };
  ...
  return { passed: score >= cslThreshold, score, reason: score < cslThreshold ? `Score ${score.toFixed(3)} < ${cslThreshold}` : undefined };
}

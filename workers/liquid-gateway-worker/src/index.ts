...
async function serveSite(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // GitHub Pages 404 — fall back to Cloud Run dynamic-site-server
    ...
    // GitHub Pages unreachable — fall back to Cloud Run
    return proxyToCloudRun(request, url, env);
  }
}

// ── Cloud Run Dynamic Site Proxy ────────────────────────────────────
async function proxyToCloudRun(request: Request, url: URL, env: Env): Promise<Response> {
  const hostname = new URL(request.url).hostname;
  const cloudRunOrigin = 'https://headyme-site-bf4q4zywhq-uc.a.run.app';
  try {
    const proxyUrl = `${cloudRunOrigin}${url.pathname}${url.search}`;
    const proxyResponse = await fetch(proxyUrl, {
      method: request.method,
    ...
    return new Response(body, {
      ...
      headers: {
        'Content-Type': proxyResponse.headers.get('Content-Type') || 'text/html; charset=utf-8',
        'X-Heady-Gateway': 'cloud-run-fallback',
        'X-Heady-Host': hostname,
      },
    });
  } catch (err) {

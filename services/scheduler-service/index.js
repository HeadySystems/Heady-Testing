...
class SchedulerService {
    ...
    async handleListJobs(req, res) {
        ...
        try {
            ...
            this.logger.error({ component: 'scheduler', action: 'list_jobs', requestId, error }, 'Failed to list jobs');
            throw error;
        }
    }
    async handleCancelJob(req, res) {
        ...
        try {
            ...
            this.logger.error({ component: 'scheduler', action: 'cancel_job', requestId, error }, 'Failed to cancel job');
            throw error;
        }
    }
    async handleTriggerJob(req, res) {
        ...
        try {
            ...
            this.logger.error({ component: 'scheduler', action: 'trigger_job', requestId, error }, 'Failed to trigger job');
            throw error;
        }
    }
    async handleHealth(_req, res) {
        ...
        try {
            ...
            this.logger.error({ component: 'health', error, durationMs: Date.now() - startTime }, 'Health check failed');
            const health = {
                status: 'unhealthy',
                timestamp: new Date().toISOString(),
                version: '1.0.0',
    ...
    async start() {
        try {
            this.app.listen(this.port, () => {
                this.logger.info({ component: 'server', port: this.port, environment: process.env.NODE_ENV }, 'Scheduler service started');
            });
        }
        catch (error) {

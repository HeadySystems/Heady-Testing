...
mcp.tool(
    'health_check',
    'Check HeadyOracle health and graph topology',
    {},
    async () => ({
        content: [{
            type: 'text',
...
mcp.tool(
    'heady_oracle_query',
    'Traverse the knowledge graph with natural language — find connections humans miss',
    {
        query: { type: 'string', description: 'Natural language graph query' },
        max_hops: { type: 'number', description: 'Maximum traversal depth' },
        include_inferred: { type: 'boolean', description: 'Include AI-inferred edges' },
    },
    async ({ query, max_hops = 5, include_inferred = true }) => {
...
mcp.tool(
    ...
    'Create typed relationships between entities in the knowledge graph',
    {
        ...
        confidence: { type: 'number', description: 'Edge confidence score' },
        bidirectional: { type: 'boolean', description: 'Create edge in both directions' },
    },
    async ({ from_entity, to_entity, relationship, confidence = 0.809, bidirectional = false }) => {
        const edge = {
        ...
        // Auto-register nodes
        if (!nodes.has(from_entity)) nodes.set(from_entity, { id: from_entity, edges: [] });
        if (!nodes.has(to_entity)) nodes.set(to_entity, { id: to_entity, edges: [] });
...
mcp.tool(
    'heady_oracle_discover',
    'Find hidden patterns and emergent clusters in the knowledge graph',
    {
        seed_entity: { type: 'string', description: 'Starting entity for discovery' },
        discovery_mode: { type: 'string', description: 'shortest_path | community | influence | anomaly' },
        depth: { type: 'number', description: 'Discovery depth' },
    },
    async ({ seed_entity, discovery_mode = 'community', depth = 3 }) => ({
        content: [{
            type: 'text',
            text: JSON.stringify({
                discovery: {
...
mcp.tool(
    'heady_oracle_explain',
    'Generate human-readable explanations of graph paths and reasoning chains',
    {
        from: { type: 'string', description: 'Source entity' },
        to: { type: 'string', description: 'Target entity' },
        style: { type: 'string', description: 'technical | narrative | visual' },
    },
    async ({ from, to, style = 'narrative' }) => ({
        content: [{
            type: 'text',
            text: JSON.stringify({
                explanation: {
...
mcp.tool(
    'heady_oracle_infer',
    'Run graph neural inference to discover new relationships',
    {
        scope: { type: 'string', description: 'full | neighborhood | entity_type' },
        min_confidence: { type: 'number', description: 'Minimum inference confidence' },
        max_inferences: { type: 'number', description: 'Maximum new inferences to generate' },
    },
    async ({ scope = 'neighborhood', min_confidence = PSI, max_inferences = 21 }) => {
        const newInferences = [];
...
mcp.resource(
    'heady://oracle/graph',
    'heady://oracle/graph',
    'Full knowledge graph statistics and topology',
    'application/json',
    async () => ({
        contents: [{
...
mcp.resource(
    'heady://oracle/entities',
    'heady://oracle/entities',
    'Entity registry with types and cardinality',
    'application/json',
    async () => ({
        contents: [{
...
mcp.resource(
    'heady://oracle/inferences',
    'heady://oracle/inferences',
    'Recently inferred relationships awaiting validation',
    'application/json',
    async () => ({
        contents: [{
...
serve({ fetch: app.fetch, port: PORT }, () => {
    console.log(`[HeadyOracle] Knowledge Graph Engine projected on :${PORT}`);
    console.log(`[HeadyOracle] SSE: http://localhost:${PORT}/sse`);
    console.log(`[HeadyOracle] Health: http://localhost:${PORT}/health`);
});

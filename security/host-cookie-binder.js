    ...
    (function() {
      ...
      window.addEventListener('message', function(event) {
        if (allowedOrigins.indexOf(event.origin) === -1) {
          process.stdout.write(JSON.stringify({ ts: new Date().toISOString(), level: 'warning', service: 'host-cookie-binder', msg: 'Rejected message from unauthorized origin', origin: event.origin }) + '\n');
          return;
        }

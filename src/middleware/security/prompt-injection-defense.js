 ...
 * ════════════════════════════════════
 * OWASP Top 10 for AI — defense against prompt injection, jailbreaking,
 * and adversarial input on every LLM-touching endpoint.
 *
 * Features:
 *  - Pattern-based injection detection (prefix, suffix, override attacks)
 *  - Parameterized prompt templates (not string concatenation)
 *  - Output validation against JSON schema
 *  - CSL confidence gating on suspicious inputs
 *  - DOMPurify-compatible HTML sanitization for outputs
 *
 * φ-derived: Suspicion threshold = PSI² ≈ 0.382
 *            Block threshold = PSI ≈ 0.618
 */
...
const INJECTION_PATTERNS = [
    ...
    { pattern: /ignore\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|prompts?|rules?|context)/i, weight: 0.9, category: 'override' },
    { pattern: /forget\s+(everything|all|your)\s*(instructions?|training|rules?|context)?/i, weight: 0.85, category: 'override' },
    { pattern: /disregard\s+(all|your|the)\s*(previous|prior|system)?\s*(instructions?|prompts?|rules?)/i, weight: 0.9, category: 'override' },

    // Role manipulation
    { pattern: /you\s+are\s+now\s+(a|an|the)\s+/i, weight: 0.7, category: 'role' },
    { pattern: /pretend\s+(to\s+be|you\s+are|you're)\s/i, weight: 0.75, category: 'role' },
    { pattern: /act\s+as\s+(if\s+you\s+are|a|an|the)\s/i, weight: 0.6, category: 'role' },
    { pattern: /your\s+new\s+(role|persona|identity|character)\s+is/i, weight: 0.85, category: 'role' },

    // System prompt extraction
    { pattern: /repeat\s+(your|the)\s+(system\s+)?(prompt|instructions?|rules?|initial)/i, weight: 0.95, category: 'extraction' },
    { pattern: /what\s+(are|is)\s+your\s+(system\s+)?(prompt|instructions?|rules?)/i, weight: 0.8, category: 'extraction' },
    { pattern: /show\s+(me\s+)?(your|the)\s+(system\s+)?(prompt|instructions?|rules?|config)/i, weight: 0.9, category: 'extraction' },
    { pattern: /output\s+(your|the)\s+(system\s+)?(prompt|instructions?|initialization)/i, weight: 0.9, category: 'extraction' },

    // Delimiter attacks
    { pattern: /```system/i, weight: 0.85, category: 'delimiter' },
    { pattern: /\[SYSTEM\]|\[INST\]|\[\/INST\]|<\|system\|>|<\|user\|>|<\|assistant\|>/i, weight: 0.9, category: 'delimiter' },
    { pattern: /###\s*(system|instruction|human|assistant)\s*:/i, weight: 0.8, category: 'delimiter' },

    // Encoding/obfuscation
    { pattern: /base64\s*(encode|decode|convert)/i, weight: 0.5, category: 'encoding' },
    { pattern: /rot13|caesar\s*cipher|hex\s*encode/i, weight: 0.6, category: 'encoding' },

    // Data exfiltration
    { pattern: /send\s+(this|the|all)\s+(data|info|information|response)\s+to\s/i, weight: 0.8, category: 'exfiltration' },
    { pattern: /fetch\s*\(\s*['"]https?:\/\/(?!.*headysystems)/i, weight: 0.7, category: 'exfiltration' },
    { pattern: /curl\s+|wget\s+|nc\s+/i, weight: 0.6, category: 'exfiltration' },
];
...
class PromptTemplate {
     ...
     * @param {object} schema   - Parameter schema for validation
     */
    ...
    /**
     * Render the template with sanitized parameters.
     * @param {object} params - Template parameter values
     * @returns {string} Rendered prompt with sanitized inputs
     */
    render(params = {}) {
    ...
    get paramNames() { return [...this._paramNames]; }
}
...
function _sanitizeParam(value, schema = {}) {
    ...
    sanitized = sanitized
        ...
        .replace(/###\s*(system|instruction|human|assistant)\s*:/gi, '');
...
function analyzeInput(text) {
    if (!text || typeof text !== 'string') {
        return { score: 0, patterns: [], blocked: false, category: null };
    }
    ...
    return {
        ...
        blocked: score >= PSI, // Block at φ⁻¹ ≈ 0.618

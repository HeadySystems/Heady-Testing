/**
 * Heady™ LiquidSelfHeal v1.0
 * Self-healing infrastructure with learned failure rules
 * Extends Temporal's durable execution with pattern learning
 *
 * Learns failure patterns from Event History and automatically
 * generates healing rules. Preemptively reroutes before failure.
 * Auto-scales resources for known patterns.
 * Rules stored as versioned policies with confidence scores.
 *
...
const HEAL_ACTIONS = Object.freeze({
  REROUTE:        'REROUTE',        // switch to alternate provider
  SCALE_UP:       'SCALE_UP',       // increase resource limits
  CIRCUIT_OPEN:   'CIRCUIT_OPEN',   // trip circuit breaker preemptively
  RESTART:        'RESTART',        // restart service
  CACHE_WARM:     'CACHE_WARM',     // pre-warm cache
  TOKEN_REFRESH:  'TOKEN_REFRESH',  // refresh auth tokens
  BACKOFF:        'BACKOFF',        // increase intervals
...
class LiquidSelfHeal extends EventEmitter {
  ...
  async recordFailure(category, service, error, context = {}) {
    ...
    const health = this._serviceHealth.get(service) || { healthy: true, lastCheck: 0, failures: 0 };
    health.failures++;
    ...
    if (matchedRule && matchedRule.enabled) {
      ...
      return { event, rule: matchedRule, healed };
    }

    // Try to learn new rule
    this._learnFromHistory(event);

    this.emit('failure:recorded', { event, matched: !!matchedRule });
    return { event, rule: null, healed: false };
  }
  ...
  recordHealFailure(ruleId) {
    ...
    if (rule) {
      ...
      if (rule.confidence < CSL_THRESHOLDS.MINIMUM && rule.failures > fib(5)) {
        rule.enabled = false;
        logger.warn({ ruleId, confidence: rule.confidence }, 'Rule disabled due to low confidence');
      }
    }
  }
  ...
  async _applyRule(rule, event) {
    ...
    if (!callback) {
      logger.warn({ action: rule.action }, 'No callback registered for heal action');
      this.emit('heal:no_callback', { action: rule.action });
      return false;
    }
    ...
    try {
      ...
      this.emit('heal:applied', { ruleId: rule.id, action: rule.action, service: event.service });
      logger.info({ ruleId: rule.id, action: rule.action, service: event.service }, 'Healing applied');
      return true;

    } catch (e) {
      logger.error({ ruleId: rule.id, error: e.message }, 'Heal action failed');
      return false;
    }
  }
  ...
  _learnFromHistory(latestEvent) {
    ...
    const relevant = this._failureLog.filter(e =>
      e.timestamp >= windowStart &&
      e.category === latestEvent.category &&
      e.service === latestEvent.service
    );

    if (relevant.length < MIN_OCCURRENCES) return;

    // Check if we already have a rule for this pattern
    const existingRule = [...this._rules.values()].find(r =>
      r.pattern.category === latestEvent.category &&
      r.pattern.service === latestEvent.service
    );
    ...
    this.emit('rule:learned', { ruleId: rule.id, name: rule.name, confidence: rule.confidence });
    logger.info({ ruleId: rule.id, name: rule.name, confidence: rule.confidence }, 'New healing rule learned');
  }
  ...
  getRules() {
    return [...this._rules.values()].map(r => ({
      ...
      successRate: r.successRate, enabled: r.enabled,
...
module.exports = { LiquidSelfHeal, FAILURE_CATEGORIES, HEAL_ACTIONS };

...
function createLatentOS(opts = {}) {
    ...
    pipeline.on && pipeline.on('stored', (entry) => {
        try { spatialMapper.add && spatialMapper.add(entry.key, entry.vector); } catch(_) { /* absorbed: */ console.error(_.message); }
    });

    awareness.on && awareness.on('drift-detected', (data) => {
        try { driftDetector.onDriftSignal && driftDetector.onDriftSignal(data); } catch(_) { /* absorbed: */ console.error(_.message); }
    });

...
const PHRASING = {
  PIPELINE_CYCLE_COMPLETE: refs => `Pipeline cycle #${refs.cycleNumber || "?"} completed. Gates: ${refs.gatesSummary || "all passed"}.`,
  PIPELINE_GATE_FAIL: refs => `Pipeline gate "${refs.gate || "unknown"}" failed. Reason: ${refs.reason || "unspecified"}.`,
  BUILD_FAILED: refs => `Build #${refs.buildId || "?"} failed due to ${refs.reason || "unknown errors"}.`,
  BUILD_SUCCESS: refs => `Build #${refs.buildId || "?"} succeeded.`,
  DEPLOY_SUCCESS: refs => `Deployment to ${refs.target || "production"} completed successfully.`,
  DEPLOY_FAILED: refs => `Deployment to ${refs.target || "production"} failed: ${refs.reason || "unknown"}.`,
  ARENA_WINNER_CHOSEN: refs => `Arena Mode selected ${refs.candidate || "winner"} (${refs.score || "?"}% pass probability) and squashed into main.`,
  ARENA_SQUASH_MERGE: refs => `Winner candidate squash-merged into main. Old branches archived.`,
  RESOURCE_CRITICAL: refs => `Resources critical: ${refs.resourceType || "system"} at ${refs.percent || "?"}%. ${refs.mitigation || "Mitigation applied"}.`,
  RESOURCE_SAFE_MODE_ENTER: () => `System entered safe mode to protect interactive responsiveness.`,
  RESOURCE_SAFE_MODE_EXIT: () => `System exited safe mode. Resources stabilized.`,
  SCHEMA_MIGRATED: refs => `Schema migrated: ${refs.description || "update applied"}.`,
  NODE_ACTIVATED: refs => `Node "${refs.nodeId || "?"}" activated.`,
  NODE_DEACTIVATED: refs => `Node "${refs.nodeId || "?"}" deactivated.`,
  PATTERN_ADDED: refs => `New pattern added: "${refs.patternName || "?"}".`,
  PATTERN_DEPRECATED: refs => `Pattern deprecated: "${refs.patternName || "?"}".`,
  USER_DIRECTIVE: refs => `User directive: "${refs.directive || refs.description || "?"}"`,
  USER_PIVOT: refs => `Project pivot: "${refs.reason || refs.description || "?"}"`,
  USER_ANNOTATION: refs => `Note: "${refs.text || refs.description || "?"}"`
};
...
const ALWAYS_INCLUDE = new Set(["BUILD_FAILED", "ARENA_WINNER_CHOSEN", "PIPELINE_GATE_FAIL", "RESOURCE_CRITICAL", "SCHEMA_MIGRATED", "USER_DIRECTIVE", "USER_PIVOT"]);
function uid() {
...
class HCStoryDriver extends EventEmitter {
  ...
  ingestSystemEvent({
    ...
    let target = Object.values(this.stories).find(s => s.status === "ongoing" && s.scope === scope);
    if (!target) {
    ...
    return this.addEvent(target.id, {
      type,
      refs,
      source
    });
  ...
  _generateSummary(story, level = "standard") {
    ...
    if (selected.length < max) {
      const remaining = story.timeline.filter(e => !selected.find(s => s.id === e.id)).slice(-(max - selected.length));
...
function registerStoryRoutes(app, storyDriver) {
  ...
  app.get(prefix, (req, res) => {
    ...
    res.json({
      ok: true,
      stories: storyDriver.listStories({
        scope,
        status
      }),
      ts: new Date().toISOString()
  ...
  app.get(`${prefix}/recent`, (req, res) => {
    ...
    res.json({
      ok: true,
      events: storyDriver.getRecentEvents(limit),
      ts: new Date().toISOString()
    });
  });
  ...
  app.get(`${prefix}/:storyId/summary`, (req, res) => {
    ...
    if (!summary) return res.status(404).json({
      error: "Story not found"
    });
    res.json({
      ok: true,
  ...
  app.post(prefix, (req, res) => {
    ...
    const story = storyDriver.createStory({
      scope,
      title,
      links
    });
  ...
  app.post(`${prefix}/:storyId/events`, (req, res) => {
    ...
    const event = storyDriver.addEvent(req.params.storyId, {
      type,
      refs,
      severity,
      source
    ...
    if (!event) return res.status(404).json({
      error: "Story not found or event filtered"
    });
    res.status(201).json({
      ok: true,
      ...event
  ...
  app.post(`${prefix}/:storyId/pin/:eventId`, (req, res) => {
    ...
    if (!pins) return res.status(404).json({
      error: "Story not found"
    });
    res.json({
      ok: true,

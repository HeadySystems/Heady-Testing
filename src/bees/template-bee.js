...
function heady_auth(provider){
  ...
  try{sessionStorage.setItem('heady_auth_session',JSON.stringify({ts:Date.now(),site:'${domain}',provider:provider}));}catch (e) { logger.error('Operation failed', { error: e.message }); }
...
(function(){
  ...
  if(s){try{var d=JSON.parse(s);if(Date.now()-d.ts<86400000){var g=document.getElementById('heady-auth-gate');if(g){g.style.display='none';}}}catch (e) { logger.error('Operation failed', { error: e.message }); }}
})();
<\\/script>`;
}

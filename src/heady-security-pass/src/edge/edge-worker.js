...
const _edgeAllowedOrigins = ['https://headyme.com', 'https://headysystems.com', 'https://headyai.com', 'https://app.headyai.com'];

function _corsResponse(response, request) {
  const headers = new Headers(response.headers);
  const reqOrigin = request?.headers?.get?.('Origin') || '';
  const allowedOrigin = _edgeAllowedOrigins.includes(reqOrigin) ? reqOrigin : _edgeAllowedOrigins[0];

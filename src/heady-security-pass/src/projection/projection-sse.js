    ...
    closeAll() {
        for (const client of this._clients.values()) {
            try {
                ...
                logger.warn({ event: 'sse_close_failed', clientId: client.id, error: err.message });
            }
        }
        this._clients.clear();
...
function createSSERouter(projectionManager) {
    ...
    router.get('/', (req, res) => {
        ...
        const _pOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyai.com,https://app.headyai.com')
            .split(',').map(s => s.trim()).filter(Boolean);
        const _pReqOrigin = req.headers.origin;
        res.setHeader('Access-Control-Allow-Origin', _pReqOrigin && _pOrigins.includes(_pReqOrigin) ? _pReqOrigin : _pOrigins[0]);
        ...
        if (projectionManager && typeof projectionManager.getAllProjections === 'function') {
            try {
                ...
                logger.warn({ event: 'sse_initial_sync_failed', error: err.message });
            }
        }

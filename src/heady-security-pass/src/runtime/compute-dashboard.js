...
function registerRoutes(app, orchestrator) {
    ...
    app.get("/api/compute/stream", (req, res) => {
        const _cdOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyai.com,https://app.headyai.com')
            .split(',').map(s => s.trim()).filter(Boolean);
        const _origin = req.headers.origin;
        const _corsOrigin = _origin && _cdOrigins.includes(_origin) ? _origin : _cdOrigins[0];
        res.writeHead(200, {

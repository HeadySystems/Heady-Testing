...
class HeadyApiGatewayV2 extends EventEmitter {
  ...
  get mesh() {
    if (!this._mesh) {
      ...
      catch (e) { process.stderr.write(JSON.stringify({ event: 'optional_dep_missing', dep: 'heady-service-mesh', error: e.message, ts: new Date().toISOString() }) + '\n'); }
    }
    return this._mesh;
  }
  ...
  get obs() {
    if (!this._obs) {
      ...
      catch (e) { process.stderr.write(JSON.stringify({ event: 'optional_dep_missing', dep: 'heady-observability', error: e.message, ts: new Date().toISOString() }) + '\n'); }
    }
    return this._obs;
  }
  ...
  get cfg() {
    if (!this._cfg) {
      ...
      catch (e) { process.stderr.write(JSON.stringify({ event: 'optional_dep_missing', dep: 'heady-config-server', error: e.message, ts: new Date().toISOString() }) + '\n'); }
    }
    return this._cfg;
  }
  ...
  get bus() {
    if (!this._bus) {
      ...
      catch (e) { process.stderr.write(JSON.stringify({ event: 'optional_dep_missing', dep: 'heady-event-bus', error: e.message, ts: new Date().toISOString() }) + '\n'); }
    }
    return this._bus;
  }
  ...
  async start() {
    ...
    process.stdout.write(JSON.stringify({ event: 'gateway_v2_started', port, service: this._opts.serviceName, ts: new Date().toISOString() }) + '\n');
    return this;
  }
  ...
  _corsMiddleware() {
    return (req, res, next) => {
      ...
      res.setHeader('Access-Control-Allow-Origin',  allowed ? (origin || 'null') : 'null');
...
if (require.main === module) {
  (async () => {
    ...
    const shutdown = async (sig) => {
      process.stdout.write(JSON.stringify({ event: 'gateway_v2_shutdown', signal: sig, ts: new Date().toISOString() }) + '\n');
      await gw.stop();
      process.exit(0);
    };
    process.on('SIGTERM', () => shutdown('SIGTERM'));
    process.on('SIGINT',  () => shutdown('SIGINT'));
  })().catch((err) => { process.stderr.write(JSON.stringify({ event: 'gateway_v2_fatal', error: err.message, ts: new Date().toISOString() }) + '\n'); process.exit(1); });
}

...
const server = http.createServer((req, res) => {
  ...
  const _dsOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyme.com,https://headysystems.com').split(',').map(s => s.trim());
  const _dsOrigin = req.headers.origin && _dsOrigins.includes(req.headers.origin.replace(/\/$/, '')) ? req.headers.origin.replace(/\/$/, '') : _dsOrigins[0];
  res.setHeader('Access-Control-Allow-Origin', _dsOrigin);

...
function registerRoutes(app, orchestrator) {
    ...
    const _sdkOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyme.com,https://headysystems.com').split(',').map(s => s.trim());
    app.get("/api/events/stream", (req, res) => {
        const _sdkO = req.headers.origin && _sdkOrigins.includes(req.headers.origin.replace(/\/$/, '')) ? req.headers.origin.replace(/\/$/, '') : _sdkOrigins[0];
        res.writeHead(200, {
            "Content-Type": "text/event-stream",
            "Cache-Control": "no-cache",
            Connection: "keep-alive",
            "Access-Control-Allow-Origin": _sdkO,
            "Vary": "Origin",
        });

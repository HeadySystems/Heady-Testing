...
let VectorMemory;
try { ({ VectorMemory } = require('../vector-memory')); } catch (_) { logger.debug('[AutoContext] VectorMemory not available: ' + _.message); VectorMemory = null; }

let cosineSimilarity;
try { ({ cosineSimilarity } = require('../vector-space-ops')); } catch (_) { logger.debug('[AutoContext] cosineSimilarity not available: ' + _.message); cosineSimilarity = null; }
...
class HeadyAutoContext extends EventEmitter {
        ...
        try {
            ...
            for (const dir of watchDirs) {
                try {
                    ...
                    if (w.unref) w.unref();
                } catch (_) { logger.debug('[AutoContext] dir not watchable: ' + dir); }
            }
        } catch (_) { logger.debug('[AutoContext] fs.watch not available: ' + _.message); }
    ...
    _gatherPriorPatterns(task) {
        ...
        } catch (_) { logger.warn('[AutoContext] _gatherPriorPatterns failed: ' + _.message); }
    ...
    _gatherDomainContext(domain) {
        ...
        for (const dir of dirs) {
            ...
            } catch (_) { logger.warn('[AutoContext] _gatherDomainContext readdir failed for ' + dir + ': ' + _.message); }
        }

...
class MCPTransport extends EventEmitter {
  ...
  handleSSE(req, res) {
    ...
    const _mcpOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyai.com,https://app.headyai.com')
        .split(',').map(s => s.trim()).filter(Boolean);
    const _mcpReqOrigin = req.headers.origin;
    res.setHeader('Access-Control-Allow-Origin', _mcpReqOrigin && _mcpOrigins.includes(_mcpReqOrigin) ? _mcpReqOrigin : _mcpOrigins[0]);

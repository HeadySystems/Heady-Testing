...
const _mcpBridgeOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyai.com,https://app.headyai.com').split(',').map(s => s.trim());

function _mcpBridgeCorsOrigin(req) {
    const origin = req.headers.origin || req.headers.referer;
    if (origin && _mcpBridgeOrigins.includes(origin.replace(/\/$/, ''))) return origin.replace(/\/$/, '');
    return _mcpBridgeOrigins[0];
}
...
function handleSSE(req, res) {
    ...
    res.writeHead(200, {
        ...
        'Access-Control-Allow-Origin': _mcpBridgeCorsOrigin(req),
        'Vary': 'Origin',
    });
...
function jsonRes(res, code, data, req) {
    const origin = req ? _mcpBridgeCorsOrigin(req) : _mcpBridgeOrigins[0];
    res.writeHead(code, {
        'Content-Type': 'application/json',

...
class CORSRouteRegistry {
   ...
   * @param {boolean}  [corsOptions.allowAll]      - Reflect allowed public origins without credentials
   */
...
function corsPolicy(opts = {}) {
  ...
  return (req, res, next) => {
    ...
    if (routeOverride?.allowAll) {
      const publicOriginDomains = [
        ...additionalDomains,
        ...(routeOverride.origins || []).map((value) => {
          try {
      ...
      const publicOverride = validateOrigin(origin, {
        additionalDomains: publicOriginDomains,
        allowLocalhost,
      });

      if (!publicOverride.allowed) {
        process.stderr.write(JSON.stringify({ event: 'cors_public_blocked', origin, path: req.path, ts: new Date().toISOString() }) + '\n');
    ...
    if (!allowed) {
      ...
      process.stderr.write(JSON.stringify({ event: 'cors_blocked', origin, path: req.path, ts: new Date().toISOString() }) + '\n');
      if (req.method === 'OPTIONS') {
        return res.status(403).json({ error: 'CORS: Origin not allowed', origin });
      }
 ...
 */
function publicCors(methods = ['GET', 'POST'], opts = {}) {
  const { origins = [], allowLocalhost = process.env.NODE_ENV !== 'production' } = opts;
  ...
  return (req, res, next) => {
    ...
    const validation = validateOrigin(origin, {
      additionalDomains: origins.map((value) => {
        try {
          return new URL(value).hostname;
        } catch {
          return value;
...
const routeRegistry = new CORSRouteRegistry()
  ...
  .register('/api/webhooks/', { allowAll: true, methods: ['POST'], origins: ['https://partner.example.com'] })
...
app.get('/api/public/status', publicCors(['GET'], { origins: ['https://partner.example.com'] }), (req, res) => res.json({ ok: true }));

...
const { PHI_TIMING, fib } = require('../shared/phi-math');
...
const HEALTHY_SCORE_THRESHOLD = fib(11) - fib(3);
const DEGRADED_SCORE_THRESHOLD = fib(10) - fib(5);
const HEALTH_MONITOR_POOL_MAX = fib(4);
const HEALTH_MONITOR_IDLE_TIMEOUT_MS = fib(7) * 1000;
const HEALTH_MONITOR_CONNECT_TIMEOUT_MS = fib(5) * 1000;
const HEALTH_MONITOR_REDIS_RETRIES = fib(2);
const DEFAULT_MEMORY_PCT = fib(11) + fib(1);
const DEFAULT_CPU_PCT = fib(11) - fib(4);
const DEFAULT_DISK_PCT = fib(11) - fib(3);
const DEFAULT_MAX_CONNECTIONS = fib(13);
const DEFAULT_MAX_QUEUE_DEPTH = fib(15);
...
const THRESHOLD = {
  HEALTHY:  HEALTHY_SCORE_THRESHOLD,
  DEGRADED: DEGRADED_SCORE_THRESHOLD,
};
...
class HealthMonitor extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    this.config = {
      ...
      checkInterval:  opts.checkInterval  || PHI_TIMING.CYCLE,
      thresholds: {
        memoryPct:         opts.thresholds?.memoryPct         ?? DEFAULT_MEMORY_PCT,
        cpuLoad1mPct:      opts.thresholds?.cpuLoad1mPct      ?? DEFAULT_CPU_PCT,
        diskUsedPct:       opts.thresholds?.diskUsedPct       ?? DEFAULT_DISK_PCT,
        maxConnections:    opts.thresholds?.maxConnections    ?? DEFAULT_MAX_CONNECTIONS,
        maxQueueDepth:     opts.thresholds?.maxQueueDepth     ?? DEFAULT_MAX_QUEUE_DEPTH,
  ...
  async initialize() {
    ...
    if (this.config.databaseUrl && pg) {
      ...
      this._pgPool = new Pool({
        ...
        max: HEALTH_MONITOR_POOL_MAX,
        idleTimeoutMillis: HEALTH_MONITOR_IDLE_TIMEOUT_MS,
        connectionTimeoutMillis: HEALTH_MONITOR_CONNECT_TIMEOUT_MS,
      });
    }
    ...
    if (this.config.redisUrl && redis) {
      this._redisClient = new redis(this.config.redisUrl, {
        lazyConnect: true,
        connectTimeout: HEALTH_MONITOR_CONNECT_TIMEOUT_MS,
        maxRetriesPerRequest: HEALTH_MONITOR_REDIS_RETRIES,
        enableOfflineQueue: false,
      });
      await this._redisClient.connect().catch((error) => {
        logger.warn('[HealthMonitor] Redis connection failed during initialize', {
          error: error.message,
        });
      });
    }
  ...
  async destroy() {
    ...
    await this._pgPool?.end().catch((error) => {
      logger.warn('[HealthMonitor] Failed to close PostgreSQL pool cleanly', {
        error: error.message,
      });
    });
    await this._redisClient?.quit().catch((error) => {
      logger.warn('[HealthMonitor] Failed to close Redis client cleanly', {
        error: error.message,
      });
    });
  }
  ...
  async check() {
    ...
    if (state !== STATE.HEALTHY && !this._healingLock) {
      this._triggerHealing(result).catch(err => {
        logger.error('[HealthMonitor] Self-healing error', { error: err.message });
      });
    }

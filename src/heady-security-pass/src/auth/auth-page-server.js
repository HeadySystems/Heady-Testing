...
const server = http.createServer(async (req, res) => {
  ...
  const _authOrigins = (process.env.HEADY_CORS_ORIGINS || 'https://headyme.com,https://headysystems.com').split(',').map(s => s.trim());
  const _authReqOrigin = req.headers.origin && _authOrigins.includes(req.headers.origin.replace(/\/$/, '')) ? req.headers.origin.replace(/\/$/, '') : _authOrigins[0];
  res.setHeader('Access-Control-Allow-Origin', _authReqOrigin);

...
function createGateway() {
    ...
    } catch (_) { process.stderr.write(JSON.stringify({ event: 'cli_autocontext_load_failed', error: _.message, ts: new Date().toISOString() }) + '\n'); }
...
function buildSystemContext() {
    ...
    if (fs.existsSync(registryPath)) {
        ...
        } catch (e) { process.stderr.write(JSON.stringify({ event: 'cli_domain_registry_read_failed', error: e.message, ts: new Date().toISOString() }) + '\n'); }
    }
...
async function startREPL() {
    ...
    } catch (_) { process.stderr.write(JSON.stringify({ event: 'cli_history_load_failed', error: _.message, ts: new Date().toISOString() }) + '\n'); }
    ...
    rl.on('line', async (line) => {
        ...
        } catch (_) { process.stderr.write(JSON.stringify({ event: 'cli_history_save_failed', error: _.message, ts: new Date().toISOString() }) + '\n'); }

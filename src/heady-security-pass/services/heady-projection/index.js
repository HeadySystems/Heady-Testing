 ...
 *   CONDUCTOR_URL      — HeadyConductor URL (default https://conductor.headyme.com)
...
class HeadyConductorClient {
  constructor(url) {
    this.url       = url || process.env.CONDUCTOR_URL || 'https://conductor.headyme.com';
  ...
  async heartbeat(state) {
    ...
    } catch (e) { log('debug', `Conductor heartbeat failed: ${e.message}`); }
  }
}

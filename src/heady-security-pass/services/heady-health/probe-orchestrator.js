...
const logger = getLogger('heady-health-probe-orchestrator');
...
const DEFAULT_PROBE_INTERVALS = Object.freeze({
    ping: fib(6) * 1000,
...
class ProbeOrchestrator {
    constructor(options = {}) {
        ...
        this.probeInterval = {
            ping: options.pingInterval || DEFAULT_PROBE_INTERVALS.ping,
            functional: options.functionalInterval || DEFAULT_PROBE_INTERVALS.functional,
            e2e: options.e2eInterval || DEFAULT_PROBE_INTERVALS.e2e,
            visual: options.visualInterval || DEFAULT_PROBE_INTERVALS.visual,
            sweep: options.sweepInterval || DEFAULT_PROBE_INTERVALS.sweep,
        };
        this.requestTimeouts = {
            head: options.headTimeout || DEFAULT_TIMEOUTS.head,
            get: options.getTimeout || DEFAULT_TIMEOUTS.get,
            headers: options.headersTimeout || DEFAULT_TIMEOUTS.headers,
        };
    }
     ...
     */
    async fullSweep() {
        logger.info('probe_sweep_started', {
            event: 'probe_sweep_started',
            domainCount: this.domains.length,
            intervals: this.probeInterval,
            timeouts: this.requestTimeouts,
        });
        const allResults = [];

        for (const domain of this.domains) {
            logger.info('probe_domain_started', {
                event: 'probe_domain_started',
                domain,
            });
            const probes = [
            ...
            for (const probe of probes) {
                const logLevel = probe.status === 'fail'
                    ? 'error'
                    : (probe.status === 'degraded' || probe.status === 'warning')
                        ? 'warn'
                        : 'info';
                logger[logLevel]('probe_result', {
                    event: 'probe_result',
                    domain,
                    probe: probe.name,
                    level: probe.level,
        ...
        logger.info('probe_sweep_summary', {
            event: 'probe_sweep_summary',
            summary,
        });
    ...
    _httpHead(url) {
        return new Promise((resolve, reject) => {
            ...
            const req = mod.request(url, { method: 'HEAD', timeout: this.requestTimeouts.head }, res => resolve(res.statusCode));
            req.on('error', reject);
    ...
    _httpGet(url) {
        return new Promise((resolve, reject) => {
            const mod = url.startsWith('https') ? https : http;
            const req = mod.get(url, { timeout: this.requestTimeouts.get }, res => {
                let data = '';
    ...
    _getHeaders(url) {
        return new Promise((resolve, reject) => {
            const mod = url.startsWith('https') ? https : http;
            const req = mod.get(url, { timeout: this.requestTimeouts.headers }, res => {
                resolve(res.headers);
                res.destroy();
            });
            req.on('error', reject);
...
if (require.main === module) {
    ...
    if (args.includes('--full-sweep') || args.includes('--dry-run')) {
        orchestrator.fullSweep()
            .then((result) => {
                logger.info('probe_cli_complete', {
                    event: 'probe_cli_complete',
                    summary: result.summary,
                });
                process.exit(result.summary.failing > 0 ? 1 : 0);
            })
            .catch((err) => {
                logger.error('probe_cli_failed', {
                    event: 'probe_cli_failed',
                    error: err.message,
                    stack: err.stack,
                });
                process.exit(1);
        ...
        logger.info('probe_cli_usage', {
            event: 'probe_cli_usage',
            usage: 'node probe-orchestrator.js --full-sweep | --dry-run',
        });
    }
}

module.exports = { ProbeOrchestrator, DEFAULT_PROBE_INTERVALS, DEFAULT_TIMEOUTS };

...
app.post('/cycle/maintenance', async () => {
  const maintenanceUrl = process.env.HEADY_MAINTENANCE_URL || 'http://heady-maintenance:4320';
  try {
    ...
    app.log.warn({ err: error?.message }, 'maintenance service unreachable during cycle');

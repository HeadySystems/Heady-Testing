...
const ALLOWED_ORIGINS = (process.env.HEADY_CORS_ORIGINS || 'https://headyai.com,https://app.headyai.com')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
...
function createGateway() {
  ...
  app.use((req, res, next) => {
    ...
    res.on('finish', () => {
      structuredLog({ service: 'heady-gateway', event: 'request_complete', method: req.method, url: req.originalUrl, statusCode: res.statusCode, durationMs: Date.now() - start });
    });
    next();
  });
  ...
  app.use((err, req, res, _next) => {
    structuredError({ service: 'heady-gateway', event: 'error', method: req.method, url: req.originalUrl, message: err.message, stack: err.stack });
...
if (require.main === module) {
  ...
  const shutdown = (signal) => {
    structuredLog({ service: 'heady-gateway', event: 'shutdown_requested', signal });
    server.close(() => {
      structuredLog({ service: 'heady-gateway', event: 'closed' });
      process.exit(0);
    });
    setTimeout(() => {
      structuredError({ service: 'heady-gateway', event: 'forced_shutdown', signal });
      process.exit(1);
    }, 10000);
  };

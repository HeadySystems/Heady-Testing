...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // Firebase Admin token verification gate
    // In production, HEADY_REQUIRE_FIREBASE_ADMIN=true enforces server-side verification.
    // Without firebase-admin wired, the endpoint returns 501 in strict mode.
    if (process.env.HEADY_REQUIRE_FIREBASE_ADMIN === 'true') {
      console.error(JSON.stringify({
        event: 'auth_callback_firebase_admin_not_wired',
        uid: body.uid,
        ts: new Date().toISOString(),
      }));
      return NextResponse.json(
        { error: 'Server-side token verification not yet configured' },
        { status: 501 }
      );
    }
    // WARNING: Without firebase-admin, we trust client-side Firebase Auth verification.
    // Set HEADY_REQUIRE_FIREBASE_ADMIN=true once firebase-admin is wired.
...
async function lookupUser(uid: string): Promise<UserRecord | null> {
  ...
  try {
    ...
    console.error(JSON.stringify({ event: 'lookup_user_failed', uid, error: (e as Error).message, ts: new Date().toISOString() }));
    return null;
  }
}

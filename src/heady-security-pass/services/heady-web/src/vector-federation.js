...
function structuredWarn(event, details = {}) {
  process.stderr.write(JSON.stringify({
    level: 'warn',
    event,
    service: 'heady-web-vector-federation',
...
class VectorFederation {
  ...
  async upsert(entry) {
    ...
    this._replicateToPeers('push', record).catch((err) => {
      structuredWarn('vector_replication_push_failed', {
        entryId: entry.id,
        error: err.message,
      });
    });
  ...
  startGossip() {
    ...
    this._gossipTimer = setInterval(() => {
      this._gossipCycle().catch((err) => {
        structuredWarn('vector_gossip_cycle_failed', { error: err.message });
      });
  ...
  async _gossipCycle() {
    ...
    try {
      ...
      structuredWarn('vector_gossip_pull_failed', {
        peerId: target.id,
        error: err.message,
      });
    }
  ...
  async _federatedSearch(queryEmbedding, k) {
    ...
    const peerSearches = activePeers.map(async (peer) => {
      try {
        ...
        peer.active = false;
        structuredWarn('vector_federated_search_peer_failed', {
          peerId: peer.id,
          error: err.message,
        });
      }

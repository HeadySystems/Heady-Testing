...
/* ─── Structured Logging ────────────────────────────────────── */
function structuredLog(event, data = {}) { process.stdout.write(JSON.stringify({ event, ...data, ts: new Date().toISOString() }) + '\n'); }
function structuredWarn(event, data = {}) { process.stderr.write(JSON.stringify({ level: 'warn', event, ...data, ts: new Date().toISOString() }) + '\n'); }
function structuredError(event, data = {}) { process.stderr.write(JSON.stringify({ level: 'error', event, ...data, ts: new Date().toISOString() }) + '\n'); }
...
class VerticalResolver extends EventEmitter {
  ...
  _loadRegistry() {
    ...
    try {
      ...
      if (!this._registry) {
        ...
        structuredWarn('vertical_registry_not_found', { path: this.registryPath });
        this._registry = { verticals: [] };
        this._loaded = true;
        return this._registry;
      }
      // Subsequent failure — keep stale registry
      structuredWarn('vertical_registry_reload_failed', { error: err.message });
      return this._registry;
    }
  ...
  resolveVerticalId(hostname) {
    ...
    // 4. Default — log a warning so silent HeadyMe fallback is visible
    structuredWarn('vertical_default_fallback', { hostname: hostname || '', resolved: this.defaultVertical });
  ...
  loadVerticalConfig(verticalId) {
    ...
    if (!entry) {
      structuredWarn('vertical_unknown_id', { verticalId, fallback: this.defaultVertical });
    ...
    if (entry.config_path) {
      ...
      if (fs.existsSync(configFilePath)) {
        try {
          ...
          structuredError('vertical_config_load_failed', { path: configFilePath, error: err.message });
        }
      }
    }
  ...
  middleware() {
    return (req, res, next) => {
      try {
        ...
        structuredError('vertical_router_error', { error: err.message });
        ...
        try {
          ...
          structuredError('vertical_default_config_fatal', { error: fallbackErr.message });
...
function watchRegistry(resolver) {
  ...
  try {
    watcher = fs.watch(resolver.registryPath, { persistent: false }, (eventType) => {
      if (eventType === 'change') {
        structuredLog('vertical_registry_changed');
        resolver.invalidateCache();
      }
    });
  } catch (err) {
    structuredWarn('vertical_registry_watch_failed', { error: err.message });
  }

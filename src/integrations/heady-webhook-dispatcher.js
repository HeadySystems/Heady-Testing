 ...
 *
 * HeadyWebhookDispatcher — HMAC-Signed Outbound Webhook Delivery
 * ==============================================================
 *
 * Solves gap from UNIMPLEMENTED_SERVICES_AUDIT.md:
 * "Webhook delivery — no signed outbound webhook layer for cross-service
 *  event notifications to partner services and integrations."
 *
 * Features:
 *   1. HMAC-SHA256 signed payloads (X-Heady-Signature header)
 *   2. Delivery queue with φ-exponential backoff retries
 *   3. Dead-letter queue (MAX=233 entries — fib(12))
 *   4. Per-endpoint circuit breaker integration
 *   5. Structured delivery receipts with traceId
 *   6. Express routes for runtime management
 *   7. Event-bus integration — emits webhook:delivered / webhook:failed
 *
 * Signature format:
 *   X-Heady-Signature: sha256=<hex(HMAC-SHA256(secret, body))>
 *   X-Heady-Timestamp: <unix epoch ms>
 *   X-Heady-TraceId: <traceId>
 *
 * Usage:
 *   const { getDispatcher } = require('./heady-webhook-dispatcher');
 *   const d = getDispatcher();
 *   d.register('partner-a', 'https://partner.example.com/hooks', { secret: 'xxxx' });
 *   await d.dispatch('partner-a', 'pipeline:completed', { runId: 'r-001' });
 *
...
const MAX_RETRIES         = FIB[4];    // 5 — max delivery attempts
...
function log(level, msg, data = {}) {
    const entry = { level, component: 'WebhookDispatcher', msg, ts: new Date().toISOString(), ...data };
...
class HeadyWebhookDispatcher extends EventEmitter {
    ...
    register(name, url, opts = {}) {
        ...
        this._endpoints.set(name, {
            ...
            filters: opts.filters || null,   // null = receive all events
        ...
        log('info', `Endpoint registered: ${name}`, { url });
        this.emit('endpoint:registered', { name, url });
        return this;
    }

    unregister(name) {
        this._endpoints.delete(name);
        this.emit('endpoint:unregistered', { name });
        return this;
    }
    ...
    async dispatch(eventName, payload = {}, opts = {}) {
        ...
        const targets = opts.endpointName
            ? (this._endpoints.has(opts.endpointName) ? [this._endpoints.get(opts.endpointName)] : [])
            : Array.from(this._endpoints.values()).filter(ep => ep.enabled && this._matchesFilter(ep, eventName));
    ...
    async dispatchAwait(eventName, payload = {}, opts = {}) {
        return this.dispatch(eventName, payload, { ...opts, await: true });
    }
    ...
    async _deliverJob(job) {
        ...
        try {
            ...
            this.emit('webhook:delivered', { jobId: job.jobId, endpoint: job.endpoint, event: job.eventName, traceId: job.traceId });
            ...
            return { ok: true, jobId: job.jobId, endpoint: job.endpoint, statusCode: response.status };
            ...
            if (job.attempts >= MAX_RETRIES) {
                ...
                this.emit('webhook:failed', { jobId: job.jobId, endpoint: job.endpoint, event: job.eventName, error: errMsg, finalFailure: true });
                log('warn', `Dead-lettered ${job.eventName} → ${job.endpoint}`, { jobId: job.jobId, attempts: job.attempts, error: errMsg });
                return { ok: false, jobId: job.jobId, endpoint: job.endpoint, error: errMsg, deadLettered: true };
            }
            ...
            this.emit('webhook:retry', { jobId: job.jobId, endpoint: job.endpoint, attempt: job.attempts, nextRetryAt: job.nextRetryAt });
            ...
            // Re-queue
            this._queue.push(job);
            return { ok: false, jobId: job.jobId, endpoint: job.endpoint, error: errMsg, retrying: true };
        }
    }
    ...
    _updateEndpointMetrics(name, success) {
        ...
        if (!this._metrics.byEndpoint.has(name)) {
            this._metrics.byEndpoint.set(name, { delivered: 0, failed: 0 });
        }
        ...
        if (success) {
            ...
            if (ep.consecutiveFailures >= FIB[4]) {
                ...
                this.emit('endpoint:disabled', { name, consecutiveFailures: ep.consecutiveFailures });
            }
        }
    }
    ...
    getStatus() {
        ...
        for (const [name, ep] of this._endpoints) {
            const m = this._metrics.byEndpoint.get(name) || { delivered: 0, failed: 0 };
            epStatus[name] = {
                url: ep.url,
                enabled: ep.enabled,
...
function registerWebhookRoutes(app, dispatcher) {
    ...
    app.delete('/api/webhooks/:name', (req, res) => {
        ...
        d.unregister(name);
        res.json({ ok: true, unregistered: name });
    });
    ...
    app.patch('/api/webhooks/:name/enabled', (req, res) => {
        ...
        res.json({ ok: true, name, enabled: !!enabled });
    });

    app.post('/api/webhooks/dispatch', async (req, res) => {
        try {
            const { event, payload, endpointName } = req.body || {};
            if (!event || typeof event !== 'string' || !/^[a-zA-Z0-9:_\-\.]+$/.test(event)) {
                return res.status(400).json({ ok: false, error: 'event name required (alphanumeric with :_-.)' });
            }
            const jobIds = await d.dispatch(event, payload || {}, { endpointName });
            res.json({ ok: true, event, jobCount: jobIds.length, jobIds });
        } catch (err) {
    ...
    app.post('/api/webhooks/verify-signature', (req, res) => {
        const { body, signature, secret } = req.body || {};
        if (!body || !signature || !secret) {
            return res.status(400).json({ ok: false, error: 'body, signature, and secret required' });
        }
